...
class TraversePDDLDomain(PDDLVisitor):
	...
	(py) representation of the domain file.
	"""
	...
	def __init__(self):
		...
		self._actions = dict()
		self.domain = None
		self._objectType = Type('object', None)
	...
	def visit_domain_def(self, node):
		...
		self.domain = Domain(node.name, self._types, self._predicates,
	...
	def visit_type(self, node):
		...
		if node.parent == None:
			self.set_in(node, Type(node.name, 'object'))
		else:
			self.set_in(node, Type(node.name, node.parent))
	...
	def visit_predicate(self, node):
		...
		self.set_in(node, Predicate(node.name, signature))
	...
	def visit_action_stmt(self, node):
		...
		try:
			...
			self.set_in(node, Action(node.name, signature, precond, effect, decomp))
		except:
		# Create new PDDL action and store in node.
			self.set_in(node, Action(node.name, signature, precond, effect))
	...
	def add_decomp(self, decomp, c):
		...
		if isNegative:
			decomp.dellist.add(Predicate(nextPredicate.key, signature))
		else:
			decomp.addlist.add(Predicate(nextPredicate.key, signature))
	...
	def add_precond(self, precond, c):
		...
		#precond.append(Predicate(nextPredicate.key, signature))
		if forall:
			#pred = Predicate(nextPredicate.key, signature)
			p =  Predicate(nextPredicate.key, signature)
			q = Quantifier('forall', scoped_var, p)
			precond.addlist.add(q)
			return
		if isNegative:
			precond.dellist.add(Predicate(nextPredicate.key, signature))
		else:
			precond.addlist.add(Predicate(nextPredicate.key, signature))
...
class TraversePDDLProblem(PDDLVisitor):
	...
	(py) representation of the problem file.
	"""
	def get_in(self, node):
		"""
		Helper method to access a global hash in which information for each
	...
	def visit_problem_def(self, node):
		...
		self._problemDef = Problem(node.name, self._domain, self._objects,
										init_list, goal_list)
	...
	def add_goal(self, goal, c):
		...
		#goal.append(Predicate(nextPredicate.key, signature))
		if isNegative:
			goal.dellist.add(Predicate(nextPredicate.key, signature))
		else:
			goal.addlist.add(Predicate(nextPredicate.key, signature))

	def visit_goal_stmt(self, node):
		""" Visits a PDDL-problem goal state statement."""
		formula = node.formula
		goal = Effect() #to enable delete list
